# Amp SDK TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/amp-sdk.svg?label=npm%20(stable)>)](https://npmjs.org/package/amp-sdk) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/amp-sdk)

This library provides convenient access to the Amp SDK REST API from server-side TypeScript or JavaScript.
...
```sh
npm install git+ssh://git@github.com:rahulkumaryadav-hue/amp-sdk-typescript.git
```
...
const client = new AmpSDK({
  apiKey: process.env['AMP_SDK_API_KEY'], // This is the default and can be omitted
...
1. Via the `AMP_SDK_LOG` environment variable
...
const client = new AmpSDK({
  logger: logger.child({ name: 'AmpSDK' }),
...
```ts
import AmpSDK from 'npm:amp-sdk';
